...
const colorsByLength = exports.colorsByLength = Object.keys(colors).sort((a, b) => {
...
function handleResult({
  ...
  console.log(words); // must lower case everything

  let color = words.toLowerCase(); // strip any spaces out

  color = color.replace(/\s/g, ''); // check if valid color

  if (!(0, _colors.isValidColor)(color)) return; // if valid color, show UI for that
